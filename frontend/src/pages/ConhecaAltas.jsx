...
const ConhecaAltas = () => {
  ...
  return (
    ...
    >
      <ConhecaAltasBackground theme={theme} />
      ...
      <main className="relative z-10 flex-1 min-h-0 px-6 sm:px-10 lg:px-16 flex flex-col">
        <section className="pt-8 sm:pt-10 md:pt-12 text-center">
          ...
          <h1
            className={`mt-4 font-extrabold tracking-[-0.02em] text-[clamp(2rem,4vw,46px)] text-[#F59E0B]`}
          >
            Conheça o ALTAS
          </h1>
          ...
          >
            ...
            pelos espaços do campus, visualizar plantas e entender a estrutura
            dos blocos, salas e serviços oferecidos.
          </p>
        </section>

        <section className="mt-12">
          <div className="mx-auto max-w-[1080px] grid gap-7 lg:grid-cols-[1.1fr_0.9fr] items-start">
            <div
            ...
            >
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                <Card
                  theme={theme}
                  title="Mapeamento"
                  description="Visualize plantas, blocos e salas para encontrar o que precisa de forma rápida."
                />
                <Card
                  theme={theme}
                  title="Acesso rápido"
                  description="Use filtros e descrições para chegar ao ambiente certo em poucos cliques."
                />
                <Card
                  theme={theme}
                  title="Informação"
                  description="Veja detalhes de salas, ramais, laboratórios e serviços disponíveis no campus."
                />
                <Card
                  theme={theme}
                  title="Colaboração"
                  description="Desenvolvido por estudantes, para estudantes, com foco em usabilidade e aprendizado."
                />
              </div>
            </div>
            ...
            >
              ...
              >
                <li className="flex gap-3 items-start">
                  <span className="mt-1 h-2.5 w-2.5 rounded-full bg-[#1B2F55]" />
        ...
        <section className="mt-12">
          <div
            className={`mx-auto max-w-[980px] rounded-[32px] border px-8 py-10 shadow-[0_18px_50px_rgba(15,32,68,0.12)] ${
              theme === "dark"
                ? "border-white/10 bg-white/5"
                : "border-[#2F5EA8]/15 bg-white"
            }`}
...
function Card({ theme, title, description }) {
  return (
    <div
      className={`rounded-3xl border p-5 shadow-[0_14px_30px_rgba(15,32,68,0.08)] ${
        theme === "dark"
          ? "border-white/10 bg-white/5"
          : "border-[#2F5EA8]/15 bg-white"
      }`}
    >
      <h3
        className={`text-[18px] font-semibold ${theme === "dark" ? "text-white" : "text-[#1B2F55]"}`}
      >
        {title}
      </h3>
      <p
        className={`mt-3 text-[14px] leading-[1.8] ${theme === "dark" ? "text-white/70" : "text-[#1B2F55]/70"}`}
      >
        {description}
      </p>
    </div>
...
function StepCard({ theme, number, title, text }) {
  return (
    <div
      className={`rounded-3xl border p-5 ${theme === "dark" ? "border-white/10 bg-white/5" : "border-[#2F5EA8]/15 bg-white"}`}
    >
      <div className="flex h-[52px] w-[52px] items-center justify-center rounded-2xl bg-[#4A7FD4] text-white font-semibold">
        {number}
      </div>
      <h3
        className={`mt-5 text-[17px] font-semibold ${theme === "dark" ? "text-white" : "text-[#1B2F55]"}`}
      >
        {title}
      </h3>
      <p
